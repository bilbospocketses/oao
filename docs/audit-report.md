...
**Resolution status (as of the 2026-04-21 audit fix pack):** 29 of 30 findings
resolved. Only **SEC-03** (CSP `'unsafe-inline'`) remains open — deferred as a
Blazor Server inline-script constraint and tracked in the project backlog
(TODO #5). Each finding below carries a **✅ Resolved** or **⏳ Deferred** marker.
...
- Of the three critical findings, SEC-01 (GET-based signin endpoint) and SEC-02 (SignalR hub container-ID validation gap) were resolved in the 2026-04-21 fix pack; only SEC-03 (incomplete CSP — `'unsafe-inline'`) remains open, deferred as a Blazor Server constraint and tracked as TODO #5.
